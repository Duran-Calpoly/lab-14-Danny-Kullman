...
def heapify_down(heap: MinHeap, index: int) -> MinHeap:
    ...
    if left < current_size and heap_data[left] < heap_data[min]:
        min = left

    if right < current_size and heap_data[right] < heap_data[min]:
        min = right

    if min != index:
        new_heap_data = heap_data[:]
        new_heap_data[index], new_heap_data[min] = new_heap_data[min], new_heap_data[index]
        return heapify_down(MinHeap(new_heap_data), min)
    else:
        return heap  # No swap needed, return the heap as is
